    ...
    return (
      <button
        ...
        className={cn(
          ...
          active 
            ? "text-[var(--color-ink-main)] font-medium" 
            : "text-[var(--color-ink-muted)] hover:text-[var(--color-ink-main)] hover:bg-black/[0.03]"
        )}
      >
        {active && (
        ...
        <div className="flex items-center gap-2.5 relative z-10">
          <Icon className={cn("w-4 h-4 transition-colors", active ? "text-[var(--color-accent-cobalt)]" : "text-[var(--color-ink-faint)] group-hover:text-[var(--color-ink-muted)]")} />
          <span>{label}</span>
        </div>
        {badge !== undefined && (
          <span className={cn(
            "text-[11px] px-1.5 py-0.5 rounded-full relative z-10 transition-colors font-medium",
            active ? "text-[var(--color-accent-cobalt)]" : "text-[var(--color-ink-faint)] group-hover:text-[var(--color-ink-muted)]"
          )}>
            {badge}
          </span>
        )}
    ...
    <div className="w-64 flex-shrink-0 bg-transparent flex flex-col h-full border-r border-black/[0.06] z-20">
      ...
      <div className="px-4 py-4">
        <button 
          ...
          className="w-full flex items-center justify-between px-3 py-2 bg-black/[0.03] hover:bg-black/[0.05] border border-black/[0.04] rounded-xl text-[13px] text-[var(--color-ink-muted)] transition-all shadow-sm outline-none focus:ring-2 focus:ring-[var(--color-accent-cobalt)]/20"
        >
          <div className="flex items-center gap-2">
            <Search className="w-4 h-4 text-[var(--color-ink-faint)]" />
            <span>Search...</span>
          </div>
          <kbd className="text-[10px] font-sans font-medium px-1.5 py-0.5 bg-white/60 border border-black/[0.05] rounded text-[var(--color-ink-muted)] shadow-sm">⌘K</kbd>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 space-y-6 pb-4 no-scrollbar">
        {/* Library */}
        <div>
          <div className="px-3 mb-1.5 text-[11px] font-semibold text-[var(--color-ink-faint)] tracking-wider uppercase">Library</div>
        ...
        {/* Scenarios (Tags) */}
        <div>
          <div className="px-3 mb-1.5 text-[11px] font-semibold text-[var(--color-ink-faint)] tracking-wider uppercase">Scenarios</div>
          <div className="space-y-0.5">
        ...
        {/* Projects */}
        <div>
          <div className="px-3 mb-1.5 text-[11px] font-semibold text-[var(--color-ink-faint)] tracking-wider uppercase">Projects</div>
          <div className="space-y-0.5">
      ...
      <div className="p-4 mt-auto">
        <button className="w-full flex items-center gap-2.5 px-3 py-2 text-[13px] text-[var(--color-ink-muted)] hover:text-[var(--color-ink-main)] hover:bg-black/[0.03] rounded-lg transition-colors outline-none">
