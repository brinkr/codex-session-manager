      ...
      <div className="p-5 border-b border-black/[0.04]">
        ...
        <div className="space-y-1">
          <button className="w-full flex items-center gap-2.5 px-2.5 py-2 text-[12px] font-medium text-[var(--color-ink-muted)] hover:text-[var(--color-ink-main)] hover:bg-black/[0.03] rounded-md transition-colors">
            <Star className={cn("w-3.5 h-3.5", session.user.starred && "fill-[#D4AF37] text-[#D4AF37]")} />
            {session.user.starred ? 'Unstar Session' : 'Star Session'}
          </button>
          <button className="w-full flex items-center gap-2.5 px-2.5 py-2 text-[12px] font-medium text-[var(--color-ink-muted)] hover:text-[var(--color-ink-main)] hover:bg-black/[0.03] rounded-md transition-colors">
      ...
      <div className="p-5 border-b border-black/[0.04]">
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className={cn(
                ...
                session.ai.summary.status === 'completed' ? "bg-emerald-100 text-emerald-700" :
                session.ai.summary.status === 'generating' ? "bg-blue-100 text-blue-700" :
                session.ai.summary.status === 'failed' ? "bg-red-100 text-red-700" :
                "bg-black/[0.04] text-[var(--color-ink-muted)]"
              )}>
                {session.ai.summary.status}
              </span>
            </div>
            {session.ai.summary.status !== 'generating' && (
              <button className="text-[11px] font-medium text-[var(--color-accent-cobalt)] hover:underline flex items-center gap-1">
                ...
                {session.ai.summary.status === 'completed' ? 'Regenerate' : 'Generate Now'}
              </button>
            )}
          </div>
          ...
          <div>
            ...
            {session.ai.tags.autoTags.length > 0 ? (
      ...
      <div className="p-5">
        ...
        <div className="space-y-2.5 text-[11px] text-[var(--color-ink-muted)]">
          <div className="flex items-center justify-between">
            ...
            <span className="font-medium">{new Date(session.user.lastOpenedAt || session.raw.updatedAt).toLocaleDateString()}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1.5"><RotateCcw className="w-3.5 h-3.5 text-[var(--color-ink-faint)]" /> Resumed</span>
            <span className="font-medium">{session.user.resumeCount} times</span>
          </div>
        </div>
      </div>
